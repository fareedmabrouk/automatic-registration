...
def check_lectures():
    ...
    return False
def page_exists():
    driver.get("https://sdb.admin.uw.edu/timeschd/uwnetid/tsstat.asp?QTRYR=AUT+2019&CURRIC=" + department)
    time.sleep(3)
    element = driver.find_elements_by_id("submit_button")
    if element:
        login()
...
#OPEN SPRING 2019 TIME SCHEDULE PAGE
...
while not found_course:
    ...
    if (curr_time > 20 or curr_time < 2) or (curr_time > 6 and curr_time < 10) :
